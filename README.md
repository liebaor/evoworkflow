...
**只安装 EVO Skills，不安装 `evo` CLI。**

因此当前如果需要使用 `evo init`、`evo doctor`、`evo evidence`、`evo finish` 等 CLI 能力，需要从 EVOworkflow 源码运行 CLI。

v0.4.2 正在补齐 **GitHub Release CLI Bootstrap**，目标是让用户可以直接安装已经构建好的 CLI，而不需要 `git clone + pnpm build`。在 GitHub Release 正式存在以前，README 不把尚未发布的 Release URL 当成可用安装命令。
...
在该 Release 真正发布以前，README 不把计划中的下载 URL 描述为已经可用。
