...
export async function runPackageSmoke(packageRoot = path.resolve('.')): Promise<void> {
  ...
  try {
    ...
    await assertPackedDistribution(path.join(packDirectory, tarball), packageRoot)
...
export async function assertPackedDistribution(tarball: string, packageRoot = path.resolve('.')): Promise<void> {
