...
每个 Skill 只在一个阶段内工作，并在产出该阶段结果后停止。用户明确调用下一个阶段。`ask-evo` 和 `evo status` 可以推荐下一步，但永远不会自动执行。
...
发现真实 checkout、构建/测试/运行/观察入口、架构权威、能力、参考实现和未知项。先报告后写入。应用初始化只创建缺失的 EVO 文件，不覆盖已有指令。
...
识别可复用机制、主模块和受影响模块、未受影响模块、预计影响范围、垂直 Slice 以及每个 Slice 的证据。

### IMPLEMENT

默认只执行一个已授权 Slice。遇到范围扩大、验收变化、架构偏离、破坏性约定、安全 Decision、破坏性数据操作、意外依赖或重复失败时停止。
...
检查规格一致性、现有模式复用、范围纪律、改动局部性、约定泄漏、不必要复杂度、实际与预期影响范围以及证据质量。评审报告问题，不静默修复。
